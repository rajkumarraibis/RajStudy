...
## 1. **Architecture & Strategy (Yannik focus)**
...
**Q10: What are risks of ADF SAP CDC connector?**
**A:**

* Needs self-hosted IR → scaling risk (mitigated via HA clusters).
* Limited throughput with many extractors → mitigate by parallel pipelines.
* Dependency on SAP Basis team → need joint ownership.
* Network/firewall issues → solve with private endpoints.
